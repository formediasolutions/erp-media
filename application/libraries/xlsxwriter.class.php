...
class XLSXWriter
{
    ...
    protected function initializeSheet($sheet_name)
    {
        ...
        $this->sheets[$sheet_name] = (object)array(
            ...
            'row_count' => $this->defaultStartRow,
    ...
    public function writeSheetRow($sheet_name, array $row)
    {
        ...
        foreach ($row as $k => $v) {
            if(!isset($sheet->cell_formats[$k])) $sheet->cell_formats[$k] = 'string';
            $this->writeCell($sheet->file_writer, $sheet->row_count, $this->defaultStartCol + $k, $v, $sheet->cell_formats[$k]);
        }
        $sheet->file_writer->write('</row>');
        $sheet->row_count++;
        $this->current_sheet = $sheet_name;
